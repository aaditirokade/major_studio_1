...
d3.json('africa.geo.json').then((geojson) => {
    ...
    d3.json('allGDPData.json').then((gdp)=>{
    
        d3.json('allENERGYData.json').then((eng)=>{
                ...
                .on('mouseover', function(d) {
                    ...
                    d3.select("#hover")
                      // .text(d.properties.name.toUpperCase() + ' (Population: ' + (d.properties.pop_est/1000000).toFixed(1) + 'Mio.)');
                        .text(eng.filter((eng) => {var ct = d.properties.name; return eng.countryName.match(ct);}));
